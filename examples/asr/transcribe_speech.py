...
from nemo.collections.asr.models import EncDecCTCModel, EncDecHybridRNNTCTCModel, EncDecMultiTaskModel, EncDecRNNTBPEModelTgtLangID, EncDecHybridRNNTCTCBPEModelTgtLangID
...
def main(cfg: TranscriptionConfig) -> Union[TranscriptionConfig, List[Hypothesis]]:
    ...
    print("\n=== CONFIGURATION TRACE ===")
    print(f"Dataset Manifest: {cfg.dataset_manifest}")
    print(f"Model Path: {cfg.model_path}")
    print(f"Pretrained Name: {cfg.pretrained_name}")
    print(f"Decoder Type: {cfg.decoder_type}")
    ...
    if hasattr(asr_model, 'encoder'):
        print(f"Encoder Type: {type(asr_model.encoder).__name__}")
    print("========================\n")
    ...
    if hasattr(asr_model, 'change_decoding_strategy') and hasattr(asr_model, 'decoding'):
        ...
        # Check if ctc or rnnt model
            ...
            print("Using RNNT Decoding Strategy")
    ...
    if isinstance(asr_model, EncDecRNNTBPEModelTgtLangID) or isinstance(asr_model, EncDecHybridRNNTCTCBPEModelTgtLangID):
        # Special case for EncDecMultiTaskModel, where the input manifest is directly passed into the model's transcribe() function
        partial_audio = False
        filepaths = cfg.dataset_manifest
        assert cfg.dataset_manifest is not None
        print(" EncDecRNNTBPEModelTgtLangID or EncDecHybridRNNTCTCBPEModelTgtLangID")
    else:
        # prepare audio filepaths and decide whether it's partial audio
        filepaths, partial_audio = prepare_audio_data(cfg)
        print("NOT EncDecRNNTBPEModelTgtLangID or EncDecHybridRNNTCTCBPEModelTgtLangID")
    ...
    # if transcripts form a tuple (from RNNT), extract just "best" hypothesis
