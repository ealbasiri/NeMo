...
def main(cfg):
    ...
    ## Analyst
    # Read manifest to get language pairs
    with open(cfg.engine.output_filename, 'r') as f:
        manifest_data = [json.loads(line) for line in f]

    # Group samples by language pairs
    lang_pair_samples = {}
    for sample in manifest_data:
        ...
        if lang_pair not in lang_pair_samples:
            lang_pair_samples[lang_pair] = []
        lang_pair_samples[lang_pair].append(sample)

    # Process each language pair with appropriate metric
    total_results = {}
    output_samples = []
    
    for lang_pair, samples in lang_pair_samples.items():
        # Write temporary manifest for this language pair
        temp_manifest = f"temp_{lang_pair}_manifest.json"
        ...
        use_wer = source_lang == target_lang  # Use WER for same language pairs

        if use_wer:
            output_manifest_w_metric, pair_results, eval_metric = cal_write_wer(
                pred_manifest=temp_manifest,
                ...
                output_filename=f"temp_output_{lang_pair}.json",
            ...
            output_manifest_w_metric, pair_results, eval_metric = cal_write_text_metric(
                pred_manifest=temp_manifest,
                gt_text_attr_name=cfg.analyst.metric_calculator.get("gt_text_attr_name", "text"),
                pred_text_attr_name=cfg.analyst.metric_calculator.get("pred_text_attr_name", "pred_text"),
                output_filename=f"temp_output_{lang_pair}.json",
        ...
        # Read and collect processed samples
        with open(f"temp_output_{lang_pair}.json", 'r') as f:
            processed_samples = [json.loads(line) for line in f]
            
            # Find best and worst samples for this language pair
            best_score = float('inf') if use_wer else float('-inf')
            worst_score = float('-inf') if use_wer else float('inf')
            best_sample = None
            worst_sample = None
            metric = "wer" if use_wer else "bleu"
            
            for sample in processed_samples:
                score = sample.get(metric, 0)
                
                if use_wer:  # WER (lower is better)
                    if score < best_score:
                        best_score = score
                        best_sample = sample
            ...
            # Print the results for this language pair
            logging.info(f"\nBest and Worst samples for {lang_pair}:")
            if best_sample:
                logging.info(f"Best {metric}: {best_score:.3f}")
                logging.info(f"Text: {best_sample.get('text', '')}")
                logging.info(f"Pred: {best_sample.get('pred_text', '')}")
            if worst_sample:
                logging.info(f"Worst {metric}: {worst_score:.3f}")
                logging.info(f"Text: {worst_sample.get('text', '')}")
                logging.info(f"Pred: {worst_sample.get('pred_text', '')}")
                
            output_samples.extend(processed_samples)

        # Store results for this language pair
        total_results[lang_pair] = {
            "metric": "wer" if use_wer else "bleu",
            "value": pair_results[eval_metric]
        }

        # Cleanup temporary files
        os.remove(temp_manifest)
        os.remove(f"temp_output_{lang_pair}.json")

    # Write final output manifest
    if cfg.analyst.metric_calculator.output_filename:
        final_output = cfg.analyst.metric_calculator.output_filename
    ...
    # Update report
    report.update({"results_by_lang_pair": total_results})
    
    # Calculate overall metrics
    wer_pairs = [pair for pair, res in total_results.items() if res["metric"] == "wer"]
    bleu_pairs = [pair for pair, res in total_results.items() if res["metric"] == "bleu"]
    
    if wer_pairs:
        avg_wer = sum(total_results[pair]["value"] for pair in wer_pairs) / len(wer_pairs)
        report["average_wer"] = avg_wer
    
    if bleu_pairs:
        avg_bleu = sum(total_results[pair]["value"] for pair in bleu_pairs) / len(bleu_pairs)
        report["average_bleu"] = avg_bleu

    # Handle metadata analysis if needed
    for target in cfg.analyst.metadata:
        if cfg.analyst.metadata[target].enable:
            occ_avg_metric = cal_target_metadata_wer(
                manifest=final_output,
                target=target,
                meta_cfg=cfg.analyst.metadata[target],
                eval_metric="wer"  # You might want to adjust this based on language pairs
            )
            report[target] = occ_avg_metric
    ...
    logging.info(pretty)
    for lang_pair, result in total_results.items():
        metric = result["metric"]
        value = result["value"] * 100
        logging.info(f"{lang_pair} {metric}: {value:.3f}%")
